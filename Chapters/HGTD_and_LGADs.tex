...
\marginpar{\flushleft Maybe plots of time walk??}
...
\begin{itemize}
    ...
    \item $\sigma_{Jitter} = \frac{N}{dV/dt} \simeq \frac{t_{rise}}{S/N}$ where $S/N$ is the signal-over-noise ratio.
    \item $\sigma_{Landau}$ depends on the thickness of the sensor (thinner is better) and the setting of the threshold voltage.
\end{itemize}

It can be noticed that the lowest jitter and time walk errors are achieved with high S/N and small rise time, i.e. thin sensors with high gain. Time walk can also be corrected with reconstruction algorithms, such as constant-fraction-discrimination (CFD).
...
\begin{figure}
    ...
    \captionsetup{width=.8\linewidth}
    \caption{Plot of Landau function (in blue) and the Landau x Gaussian convolution (in red), with identical Landau parameters and arbitrary units, both normalized to 1. @maybe not so clear@}
