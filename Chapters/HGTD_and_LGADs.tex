...
In this chapter we explain the main reasons behind the construction of HGTD and its general layout. Furthermore, the silicon sensors (LGADs) employed in the detector are explained in detail.

\section{The High Granularity Time Detector}\label{sec:HGTD}

One of the most significant updates to the ATLAS detector will be the installation of the new ITk, extending the pseudorapidity range [for tracking] up to $\eta=4$. While this extention will improve the reconstruction of physics objects, it will bring some difficulties too. 

As $\eta$ increases, tracks become more parallel to the beam and are subject to multiple scattering effects due to increased material compared to the central barrel region. This problem will be made worse by the high luminosity, as the density of collisions will grow significantly (Figure~\ref{fig:pileup_densities}) and the spacial resolution of ITk worsens at higher pseudrapidities (Figure~\ref{fig:ITk_spacial_resolution}).
...
As mentioned before in \nameref{subsec:high_luminosity_upgrade}, the HL-LHC will see an increase in the number of collisions per bunch\footref{footnote:particle_beam_bunches} and, as a direct consequence, an increase in the average density of hard-scatter verteces. 

While the Inner Detector is currently able to reconstruct charged particles with high precision up to $\eta=4$, the aforementioned effects will

with heightened pile-up effects (when a collision happens very close to one or more others, thus complicating track reconstruction).

A new way to mitigate this effect for higher pseudorapity values will be "\textit{to use high-precision timing information to distinguish between collisions occurring very close in space but well-separated in time}" \cite{CERN-LHCC-2020-007}. This is set to be accomplished by the High Granularity Time Detector (HGTD).
